...
class Profile(models.Model):
    ...
    def get_picture(self):
        no_picture = '127.0.0.1:8000/static/img/download.png'
        try:
    ...
    def get_followers(self):
        activities = Followers.objects.filter(to_user__pk=self.pk, activity_type=Followers.FOLLOW)
        followers = []
    ...
    def get_followers_count(self):
        followers_count = Followers.objects.filter(to_user__pk=self.pk, activity_type=Followers.FOLLOW).count()
        return followers_count

    def get_following(self):
        activities = Followers.objects.filter(from_user__pk=self.pk, activity_type=Followers.FOLLOW)
        following = []
        for activity in activities:
    ...
    def get_following_count(self):
        following_count = Followers.objects.filter(from_user__pk=self.pk, activity_type=Followers.FOLLOW).count()
